...
const defaultColumns = [
  ...
  },
  {
    ...
    cell: ({ getValue, column }) => {
      ...
      const textClass = text == 'Bet Success' ? 'text-[#0000FF]' : ''
      const columnWidth = column.getSize()

      return (
        <span
          className={`${textClass} px-2`}
          style={{
            whiteSpace: 'nowrap',
            overflow: 'hidden',
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return receiptID && receiptID.trim() !== '' ? (
        <span className="text-[#0000FF]"> Confirm</span>
      ) : null
    }
  }
]

export const ListEventSuccessList = () => {
  ...
  const [isClearChecked, setIsClearChecked] = useState(() => {
    const storedValue = sessionStorage.getItem('isClearChecked_SuccessList')
    return storedValue !== null ? JSON.parse(storedValue) : true
  })
  const [isScrollChecked, setIsScrollChecked] = useState(() => {
    const storedValue = sessionStorage.getItem('isScrollChecked_SuccessList')
    return storedValue !== null ? JSON.parse(storedValue) : true
  })
  ...
  const handleScrollChange = (value: boolean) => {
    ...
    sessionStorage.setItem('isScrollChecked_SuccessList', JSON.stringify(value))
  }
  ...
  const handleClearChange = (value: boolean) => {
    ...
    sessionStorage.setItem('isClearChecked_SuccessList', JSON.stringify(value))
    window.electron.ipcRenderer.send('DataClear_SuccessList', +value)
  }
  ...
  useEffect(() => {
    const handleDataSuccessList = (_, data: WaitingListType) => {
    ...
    window.electron.ipcRenderer.on('DataSuccessList', handleDataSuccessList)
  ...
  `
    ...
    link.download = `SuccessListReport${Date.now()}.html`
    link.click()
