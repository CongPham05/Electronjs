...
import { BetListType, DataPlaceBet } from '@shared/types'
...
const defaultColumns = [
  ...
  },
  {
    ...
    cell: ({ getValue, column }) => {
      ...
      return (
        <span
          className={`px-2 flex items-center justify-start ${textClass}`}
          style={{
            whiteSpace: 'nowrap',
            overflow: 'hidden',
...

  {
    ...
    size: 50,
    cell: ({ getValue }) => {
      return <span className="text-[#FF0000]"> {getValue() as string}</span>
    }
  },
  {
    accessorKey: 'speed',
...
export const ListEventBetList = () => {
  ...
  const [isClearChecked, setIsClearChecked] = useState(() => {
    const storedValue = sessionStorage.getItem('isClearChecked_BetList')
  ...
  const handleClearChange = (value: boolean) => {
    ...
    sessionStorage.setItem('isClearChecked_BetList', JSON.stringify(value))
    window.electron.ipcRenderer.send('DataClear_BetList', +value)
  }
  ...
  useEffect(() => {
    const handleDataBetList = (_, newData: BetListType) => {
