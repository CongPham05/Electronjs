...
export const ListEventWaitingList = () => {
  ...
  const [isClearChecked, setIsClearChecked] = useState(() => {
    const storedValue = sessionStorage.getItem('isClearChecked_WaitingList')
    return storedValue !== null ? JSON.parse(storedValue) : true
  })
  const [isScrollChecked, setIsScrollChecked] = useState(() => {
    const storedValue = sessionStorage.getItem('isScrollChecked_WaitingList')
    return storedValue !== null ? JSON.parse(storedValue) : true
  })
  ...
  const handleScrollChange = (value: boolean) => {
    ...
    sessionStorage.setItem('isScrollChecked_WaitingList', JSON.stringify(value))
  }
  ...
  const handleClearChange = (value: boolean) => {
    ...
    sessionStorage.setItem('isClearChecked_WaitingList', JSON.stringify(value))
    window.electron.ipcRenderer.send('DataClear_WaitingList', +value)
  }
  ...
  `
    ...
    link.download = `WaitingListReport${Date.now()}.html`
    link.click()
